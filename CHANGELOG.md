...
### Added
- Added an `angular` dependency group to `.github/dependabot.yml` covering `@angular/*` and `typescript`, so Dependabot raises one combined PR instead of one-per-package. Prevents a recurrence of the `ERESOLVE` failures that broke [#83](https://github.com/J-MaFf/PersonalWebsite/pull/83)/[#81](https://github.com/J-MaFf/PersonalWebsite/pull/81)/[#80](https://github.com/J-MaFf/PersonalWebsite/pull/80) ([#93](https://github.com/J-MaFf/PersonalWebsite/pull/93)).
...
### Changed
- Upgraded all `@angular/*` packages together to **22.1.x** (`@angular/build` `^22.1.3`, `@angular/cli` `~22.1.3`, framework packages `^22.1.1`). Angular's packages peer-depend on each other at exact versions, so Dependabot's three separate single-package bumps ([#83](https://github.com/J-MaFf/PersonalWebsite/pull/83), [#81](https://github.com/J-MaFf/PersonalWebsite/pull/81), [#80](https://github.com/J-MaFf/PersonalWebsite/pull/80)) each failed `npm ci` with `ERESOLVE`; moving them as a group resolves cleanly. `typescript` stays pinned at `~6.0.3` — `@angular/build` still peer-requires `>=6.0 <6.1` ([#91](https://github.com/J-MaFf/PersonalWebsite/pull/91)).
- Bumped `actions/checkout` and `actions/setup-node` from `v4` to **`v7`** in `ci.yml` and `deploy.yml`, clearing the Node 20 action-deprecation warning emitted on every workflow run. Verified checkout v7's fork-PR restriction does not apply — neither workflow uses `pull_request_target` or `workflow_run` ([#98](https://github.com/J-MaFf/PersonalWebsite/pull/98)).
- Raised the `anyComponentStyle` budget in `angular.json` from 2 kB/4 kB to **6 kB/12 kB**. `app.component.css` (2.80 kB) warned on every build and sat only 1.2 kB below the hard error; the stylesheet is load-bearing for the site's design, so the budget moved rather than the CSS ([#99](https://github.com/J-MaFf/PersonalWebsite/pull/99)).
- `claude.yml` now calls the shared reusable Claude workflow in [J-MaFf/.github](https://github.com/J-MaFf/.github) instead of carrying its own copy ([#87](https://github.com/J-MaFf/PersonalWebsite/pull/87)).
...
### Security
- Patched [GHSA-frvp-7c67-39w9](https://github.com/advisories/GHSA-frvp-7c67-39w9) — path traversal in `@hono/node-server`'s `serve-static` on Windows via encoded backslash (`%5C`) — with an `overrides` pin to `^2.0.5`. The advisory reached the tree transitively via `@angular/cli` → `@modelcontextprotocol/sdk` → `@hono/node-server`, surfacing as 3 moderate `npm audit` findings. `npm audit fix --force` was rejected as the remedy: it resolves the advisory by downgrading `@angular/cli` to 21.0.4, reverting the Angular 22.1.x upgrade. Back to **0 vulnerabilities** ([#97](https://github.com/J-MaFf/PersonalWebsite/pull/97)).

### Added
- Configured serving the site at the apex custom domain **`https://jmaff.dev/`**, replacing the default project-site URL `https://j-maff.github.io/PersonalWebsite/`: added `src/CNAME` (`jmaff.dev`), registered it in `angular.json` assets so it is copied into the build output on every deploy (surviving `keep_files: false` on the `gh-pages` branch), changed the deploy build from `--base-href /PersonalWebsite/` to `--base-href /`, and refreshed the stale custom-domain TODO comments in `deploy.yml`. DNS records and the repo Pages custom-domain/HTTPS settings are configured out-of-band ([#62](https://github.com/J-MaFf/PersonalWebsite/issues/62)).
- Added `vitest`, `jsdom`, and `@vitest/coverage-v8` devDependencies. `jsdom` supplies the DOM for the browserless Vitest runner — the `@angular/build:unit-test` builder requires either `jsdom` or `happy-dom` to be installed for non-browser tests, and the schematic does not add one automatically. `@vitest/coverage-v8` preserves the coverage-report capability the old `karma-coverage` setup provided, so `npm test -- --coverage` works (the migrated `coverageReporters: ["html", "text-summary"]` option needs a provider installed) ([#58](https://github.com/J-MaFf/PersonalWebsite/issues/58)).
